...
sub new
{
 ...
 $this->{'width'} =~ s/^.*?(\d*).*?$/$1/osgi;

 $this->{'height'} = $inp{'height'} || '60';
 $this->{'height'} =~ s/^.*?(\d*).*?$/$1/osgi;

 $this->{'f_max'} = $inp{'f_max'} || 32;
 $this->{'f_max'} =~ s/^.*?(\d*).*?$/$1/osgi;

 $this->{'f_min'} = $inp{'f_min'} || 14;
 $this->{'f_min'} =~ s/^.*?(\d*).*?$/$1/osgi;

 $this->{'rot'} = $inp{'rot'} || 20;
 $this->{'rot'} =~ s/^.*?(\d*).*?$/$1/osgi;

 $this->{'quality'} = $inp{'quality'} || 128;
 $this->{'quality'} =~ s/^.*?(\d*).*?$/$1/osgi;
 ...
 if (defined $this->{'cell'}) {
   if ($this->{'cell'} =~ m/yes|on/is) { $this->{'cell'} = 1 };
   if ($this->{'cell'} =~ m/no|off/is) { $this->{'cell'} = 0 };
 }
...
sub ImagePassword
{
 ...
 my $image = Image::Magick->new(compression=>'None',quality=>$quality);
 ...
 #$image->Mogrify('OilPaint', radius=>2);
 for ($i=0;$i<$len;$i++)
  {
  ...
  $image->Mogrify('Wave', geometry => '$widthx$height', amplitude => $height/15, wavelength => $width/3);
  $height += $height/5;
  $image->Mogrify('AddNoise', noise=>'Impulse');
 if($cell)
 {
  my $y;
  ...
  for ($y = 0; $y<int($height/$xstep)+1; $y++)
   {
    my $rowcolor = sprintf("#%x%x%x",rand(155)+100,rand(55)+200,rand(255));
    $image->Draw( stroke=>$rowcolor,
                  primitive=>'line',
                  points=>"0,".(($y*$xstep)+($xstep*(rand()-0.5)))
                        . ",$width,".(($y*$xstep)+($xstep*(rand()-0.55))) );
   }
  my $x;
  my $ystep = int(rand(10)+10);
  for ($x = 0; $x<int($width/$ystep)+1; $x++)
   {
    my $colcolor = sprintf("#%x%x%x",rand(100)+155,rand(100)+155,rand(200)+55);
    $image->Draw(stroke=>$colcolor,
                 primitive=>'line',
                 points=>(($x*$ystep)+($ystep*(rand()-0.5))).",0,
                 ".(($x*$ystep)+($ystep*(rand()-0.5))).",$height");
   }
 }
 $image->Mogrify('Swirl', degrees => 15);
 $image->Crop(geometry=>'$widthx$height');
 #$image->Mogrify('AddNoise');
...
{
 my $bgcolor = shift(@_);
 my @colors = ('white','black','red','blue','yellow','orange');
 my $colr;
 while (1)
  {
   $colr = rand($#colors+1);
...
=head1 SYNOPSIS
 ...
 $obj->fonts(['D:/FRENCH/FONTS/Verdana.TTF','D:/FRENCH/FONTS/Arial.TTF',
              'D:/FRENCH/FONTS/comic.TTF','D:/FRENCH/FONTS/georgiab.TTF',
              'D:/FRENCH/FONTS/micross.TTF','D:/FRENCH/FONTS/tahoma.TTF',
             ]);
